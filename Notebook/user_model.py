...
file_path = "save/" # スペクトラムの保存先
sensor_path = "../Data/センサ別npyファイル/" # センサファイルのパス(末尾にスラッシュ)
...
if step == "1":
    X_train = np.concatenate([load("Acc", "xy", nfft, overlap), load("Acc", "z", nfft, overlap),\
        load("Gyr", "xy", nfft, overlap), load("Gyr", "z", nfft, overlap),\
            load("Mag", "xy", nfft, overlap), load("Mag", "z", nfft, overlap),\
                load("LAcc", "xy", nfft, overlap), load("LAcc", "z", nfft, overlap)], axis=1)
...
# Label読むこむ
if step == "1":
    Y_train = np.load("../Data/センサ別npyファイル/train/train_Hips/train_Hips_Label.npy")[:, 0] - 1
val_label = np.load("../Data/センサ別npyファイル/validation/validation_Hips/validation_Hips_Label.npy")[:, 0] - 1
...
pattern_file = pattern_file[user_label == user]
X_val_pattern0 = X_val[user_label == user][pattern_file == 0]
X_val_pattern1 = X_val[user_label == user][pattern_file == 1]
Y_val_pattern0 = val_label[user_label == user][pattern_file == 0]
Y_val_pattern1 = val_label[user_label == user][pattern_file == 1]
...
if step == "1":
    ...
    y_train = Y_train
    y_val = Y_val_pattern1
    del X_train, Y_train
else:
    x_train = X_val_pattern1
    x_val = X_val_pattern0
    y_train = Y_val_pattern1
    y_val = Y_val_pattern0

del  X_val_pattern0, X_val_pattern1, Y_val_pattern0, Y_val_pattern1
...
def getModel(step="1"):
    if step == "1":
        ...
        model = tf.keras.models.load_model(save_folder + "model_user" + str(user) + "_" + str(nfft) + "_" + str(overlap) + "_1.hdf5")
    return model
...
cp_cb = tf.keras.callbacks.ModelCheckpoint(filepath=save_folder + "model_user" + str(user) + "_" + str(nfft) + "_" + str(overlap) + "_" + str(step) + ".hdf5", 
